...
function ProductForm({ initialData, onSubmit, loading: formLoadingProp, error: formErrorProp, isEditMode }) {
    ...
    // Trong handleFileUpload function (Sử dụng phiên bản từ snippet của bạn)
    const handleFileUpload = async (filesToUpload, contextId = 'general') => {
        ...
        try {
            ...
            // filesToUpload is typically a FileList, which has forEach in modern browsers
            ...
            if (!response.ok || !result.success || !Array.isArray(result.images)) {
                throw new Error(result.message || `Upload failed for context ${contextId}`);
            }
            ...
            color_variants_data: colorVariants
                ...
                .map(variant => ({
                    ...
                    variant_specific_images: variant.variant_specific_images.map(img => ({
                        image_url: img.uploadedUrl || img.previewUrl, // Use uploadedUrl if available
                ...
                <Col md={8}>
                                    ...
                                    <Card key={variant.tempId} className="mb-3 shadow-sm variant-card">
                                        ...
                                        <Card.Body>
                                            <div className="mb-3 p-3 border rounded bg-white">
                                                ...
                                                {/* Updated variant image upload section */}
                                                <Form.Group controlId={`variantImagesUpload-${variant.tempId}`}>
                                                    <ImageUpload
                                                        onUpload={(files) => handleVariantImageUpload(variant.tempId, files)}
                                                        disabled={isLoadingOverall || !variant.color_id}
                                                        maxFiles={5} // Max 5 images per variant color
                                                        className="mb-2"
                                                    />
                                                </Form.Group>
                                                <ListGroup variant="flush">
                                                    {/* Updated variant image rendering */}
                                                        ...
                                                        <ListGroup.Item key={img.tempId} className="px-0 py-2">
                                                            <Row className="align-items-center g-2">
                                                                <Col xs="auto" style={{ width: '70px' }}>
                                                                    <OptimizedImage
                                                                        src={img.previewUrl || PLACEHOLDER_IMAGE_PATH}
                                                                        alt={img.alt_text_vi || `Product Image ${img.display_order + 1}`}
                                                                        width={60}
                                                                        height={60}
                                                                        className="rounded"
                                                                    />
                    ...
                    <Accordion defaultActiveKey="0" className="mb-3">
                        <Accordion.Item eventKey="0">
                            ...
                            <Accordion.Body>
                                {/* Updated general gallery image upload section */}
                                <Form.Group controlId="generalGalleryImagesUploadForm" className="mb-3">
                                    ...
                                    <ImageUpload
                                        onUpload={(files) => handleGeneralGalleryImageUpload(files)}
                                        disabled={isLoadingOverall} // isLoadingOverall covers imageUploadStates['general_gallery']
                                        maxFiles={10} // Example: Max 10 general images
                                        className="mb-2"
                                    />
                                </Form.Group>
                                <ListGroup variant="flush">
                                    {generalGalleryImages.map((img) => (
                                        <ListGroup.Item key={img.tempId} className="px-0 py-2">
                                            <Row className="align-items-center g-2">
                                                <Col xs="auto" style={{ width: '70px' }}>
                                                    {/* Using OptimizedImage for general gallery too for consistency */}
                                                    <OptimizedImage
                                                        src={img.previewUrl || PLACEHOLDER_IMAGE_PATH}
                                                        alt={img.alt_text_vi || `Product Image ${img.display_order + 1}`}
                                                        width={60}
